...
/*
 * SLUB defragmentation a.k.a. SLUB Movable Objects.
 *
...
#define SMO_BUF_SIZE 40
struct smo_slub_object {
	struct list_head list;
	char buf[SMO_BUF_SIZE]; /* Unused except to control size of object */
};
			...
			int node, void *private)
{
	...
	for (i = 0; i < size; i++) {
		...
		so_new = kmem_cache_alloc(cachep, GFP_KERNEL);
		if (!so_new) {
			pr_debug("kmem_cache_alloc failded\n");
			return;
		}

		/* Nothing to do for copy old object to new */
 ...
 * @unused: (Satisfy required function signature.)
 *
 ...
 */
static int smo_alloc_objects(int n, int unused)
{
 ...
 */
static int smo_free_object(int n, int pos)
{
...
static int parse_cmd_buf(char *buf, char **cmd, long *arg1, long *arg2)
{
	...
	*arg2 = def_val_arg2;
	...
	if (ret < 0) {
		pr_err("failed to convert arg, defaulting to %ld. (%s)\n",
			def_val_arg1, ptr);
		return found;
	}
	found++;
 ...
 */
static ssize_t smo_callfn_debugfs_write(
	struct file *file, const char __user *ubuf, size_t len, loff_t *off)
{
	char *kbuf;
	int nbytes = 0;
	...
	call_function(kbuf);	/* Tokinizes kbuf */
out:
	kfree(kbuf);
	return nbytes;
}
...
static int __init smo_init(void)
{
	...
	pr_info("smo: smo test module loaded\n");
	return 0;
}
...
static void __exit smo_exit(void)
{
	...
	pr_info("smo: smo test module removed\n");
}
